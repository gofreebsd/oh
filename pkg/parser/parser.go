...
func (p *parser) ParseBuffer(interactive bool, label string, yield YieldFunc) bool {
	lines := 0
	for {
		rval, lines, e := p.parsePipe(interactive, label, lines, yield)
		if e != nil {
			c := List(
...
func (p *parser) ParseCommands(label string, yield YieldFunc) {
	if p.ParseBuffer(true, label, yield) {
		fmt.Printf("\n")
	}
}

func (p *parser) ParsePipe(label string, yield YieldFunc) (int, interface{}) {
	_, l, e := p.parsePipe(false, label, 0, yield)
...
func (p *parser) parsePipe(interactive bool, label string, start int, yield YieldFunc) (rval int, lines int, e interface{}) {
